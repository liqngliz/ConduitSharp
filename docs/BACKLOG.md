# ConduitSharp hardening backlog
...
**Fix (implemented)**: `Gateway:RequestLimits` in appsettings. `MaxRequestBodyBytes` (global per-request default, 413 when exceeded, default 8 MiB) and two independent aggregate budgets across concurrent in-flight bodies via `RequestBodyBudget`: `MaxRamBufferedBodyBytes` (heap, default 64 MiB) and `MaxDiskBufferedBodyBytes` (spill-file bytes, default 64 MiB), a 503 when a body fits neither. Routes can override the per-request limit with `maxRequestBodyBytes` in routes.json (null inherits global, `<= 0` disables for that route); the budgets are always global. Route matching now runs before body buffering, so unmatched requests (404) never buffer a byte. Enforced chunk-by-chunk so chunked bodies are capped too.
...
**What**: `PluginPipelineExecutor` records the span error and re-throws (correct for tracing), but nothing above caught it, so an unhandled plugin exception escaped the middleware with no response written, surfacing as an unobserved exception.
...
**What**: `RoundRobinLoadBalancer` and `RandomLoadBalancer` selected nodes without health awareness, so a down node was picked every N requests and ate the failure latency each time.
