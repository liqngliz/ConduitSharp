...
executable shell over that library; it ships as the `conduitsharp` dotnet tool
...
- **Response headers are one-shot.** Mutate them from `context.Response.OnStarting(...)` (fires just
  ...
  header-mutating plugins, `OnStarting` callbacks run in reverse registration order, so reverse `order`.
...
- **`PluginName.Custom`** is the open-ended extension point: a plugin declares
  ...
  Custom plugins coexist under distinct variants, with **no `Core` recompile needed** for a
...
`cluster.loadBalancingPolicy`), ASP.NET Core `MatcherPolicy` (custom route matching). Each
...
- **Integration tests**: `GatewayFactory.CreateAsync(..., plugins: [new
  FixedStatusPlugin(PluginName.JwtAuth, ...)])`; the test plugin wins because it is added
...
section, not duplicated here since it is config-facing rather than implementation detail.
...
token actually contains (check at jwt.ms, do not assume). Mixing them 401s before
...
An unassigned user's token omits the `roles` claim entirely, not an empty array. That is
...
- **Retries** apply only to idempotent methods (`GET`, `HEAD`, `OPTIONS`, `PUT`, `DELETE`,
  ...
  (`maxAttempts`, `delayMs`, `backoff` = Fixed/Linear/Exponential, `jitter`, `retryOn`). The loop
  owns what Polly cannot see: rewinding the buffered
...
The split is structural, literally the shape of the type:
...
**Retry sits *beside* the cluster, not inside it.** `ClusterConfig` has no retry field, and its
`Metadata` (an `IReadOnlyDictionary<string,string>`) could never hold a structured policy with a
status-code array, so composition is the only shape available.

**One cluster per route, `ClusterId == RouteId`.** Any code reached from YARP's side knows a
cluster id, so it can get straight back to typed ConduitSharp config with
`GatewayRouteTable.TryGetRoute(clusterId)`. Nothing travels *through* YARP, and
`ClusterConfig.Metadata` is never written to at all.

**Why YARP's types and not our own DTOs?** A hand-rolled schema is a projection that can disagree
with YARP, and it did once: `ForwarderRequestConfig` is per-cluster but the correct outbound HTTP
version depends on the *inbound* request, so the translator silently downgraded HTTP/2 and broke
gRPC, caught only by the LegacyGateway e2e suite. It also grew a field every time YARP grew one.
Using YARP's records directly deletes both problems, and every YARP feature (session affinity,
active health checks, transforms, `sslProtocols`, host matching) becomes configurable with no
work here.

**Why is the schema in `Gateway.AspNetCore` and not `Core`?** Because it is built on YARP's types,
and `Core` is what plugin authors compile against with *zero* NuGet dependencies. Plugins never
needed the route: every one only ever read `route.Id`, so the plugin contract is
...
native model and the plugin chain coexist on the same route: plugins are the data-configured,
hot-reloadable, standalone-host-usable axis alongside the policies rather than a replacement.
...
- The compiled route chain starts one `plugin.{Name}` (or `plugin.{Name}:{Variant}` for
  Custom) child span per plugin invocation, tagged `conduitsharp.plugin`, giving
...
- **`MaxRamBufferedBodyBytes`** (default 64 MiB): total heap held by buffered bodies **and
  ...
  a body that cannot get RAM spills to disk rather than being refused (charged to the disk budget).
...
under the one shared RAM budget, and never spills.
...
`ConduitSharp.Gateway.AspNetCore.Swagger` provides `app.UseConduitSharpGatewaySwagger()`,
...
of the box; embedders who don't want the Swashbuckle dependency don't reference it.
...
`routes.json`. There is no scoring or specificity algorithm, so put catch-alls last.
...
into the rest of YARP's proxy pipeline. Consequences: a plugin short-circuits by not calling
...
- `PluginAssemblyLoader`'s `ReflectionTypeLoadException` catch is intentionally
  un-covered: it requires an assembly whose type dependencies are absent, not producible
