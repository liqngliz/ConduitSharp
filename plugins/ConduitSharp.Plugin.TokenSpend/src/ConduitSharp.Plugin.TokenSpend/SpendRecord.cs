...
public sealed record SpendRecord
{
    ...
    /// <summary>Reasoning tokens the provider billed, a <em>subset</em> of <c>out</c> on the wire (the
    /// dashboard splits them into sibling Out and Think buckets). Both providers report a count whatever the
    /// content looked like, so an encrypted or redacted thinking block is measured here exactly like a
    /// plain one. Zero where the provider reports no such field, which is not the same as a model that did not think.</summary>
