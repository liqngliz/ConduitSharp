...
$GatewayDir = [IO.Path]::GetFullPath((Join-Path $Root ".." ".." "src" "ConduitSharp.Host"))
...
# ---------------------------------------------------------------------------
if ($DockerUp) {
    Write-Step "Starting dockerized LegacyGateway + Aspire Dashboard..."
    ...
    Write-Host "  ConduitSharp LegacyGateway — Docker" -ForegroundColor Yellow
    Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" -ForegroundColor Yellow
    Write-Host ""
...
if ($Stop) {
    ...
    foreach ($port in @(5050, 5060, 5101, 5102, 5201, 5301)) {
        if ($IsWindows) {
...
$procs += Start-Svc "InventoryService-1" $invDll 5101 (Join-Path $LogDir "inventory-svc-1.log")
Start-Sleep -Seconds 1
$procs += Start-Svc "InventoryService-2" $invDll 5102 (Join-Path $LogDir "inventory-svc-2.log")
Start-Sleep -Seconds 1
$procs += Start-Svc "OrderService"       $ordDll 5201 (Join-Path $LogDir "order-svc.log")
Start-Sleep -Seconds 1
$procs += Start-Svc "GreeterService"     $grtDll 5301 (Join-Path $LogDir "greeter-svc.log")
...
# Start gateway
# ---------------------------------------------------------------------------
Write-Step "Starting ConduitSharp gateway on :5050..."
...
$env:Gateway__PluginsPath                       = [IO.Path]::GetFullPath((Join-Path $Root "gateway" "plugins"))
...
Remove-Item env:Gateway__PluginsPath    -ErrorAction SilentlyContinue
...
Write-Host ""
Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" -ForegroundColor Yellow
Write-Host "  ConduitSharp LegacyGateway example running" -ForegroundColor Yellow
Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" -ForegroundColor Yellow
Write-Host ""
Write-Host "  Gateway:   http://localhost:5050"
Write-Host ""
